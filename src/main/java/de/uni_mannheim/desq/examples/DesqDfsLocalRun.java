...
public class DesqDfsLocalRun {
	...
	public static void runDistributedMiningLocally(String theCase, int scenario, int run) throws IOException{
		...
		//Tuple2<Integer, Integer> stats = miner.determinePivotElementsForSequences(inputSequences, verbose);
		Tuple2<HashMap<Integer, IntArrayList>,Tuple2<Integer, Integer>> ret = miner.createPartitions(inputSequences, verbose);
		HashMap<Integer, IntArrayList> partitions = ret._1();
		Tuple2<Integer, Integer> stats = ret._2();
		pcTime.stop();
		...
		int key;
		IntArrayList sequences;
		for(Map.Entry<Integer, IntArrayList> partition : partitions.entrySet()) {
			...
			miner.clear();
			for(int seqNo : sequences) {
				miner.addInputSequence(inputSequences.get(seqNo), 1, true);
			}
			miner.minePivot(key);
		...
		// combined print
		System.out.println("exp. no\tcase\toptimizations\trun\tcreate time\tread time\tpc time\tmine time\tno. seq\tno. piv\ttotal Recursions\ttrs used\tmxp used\tno. patterns\ttotal freq. patterns");
		String out = expNo + "\t" + theCase + "\t" + scenarioStr + "\t" + run + "\t" + prepTime.elapsed(TimeUnit.MILLISECONDS) + "\t" +
				...
				stats._1 + "\t" + stats._2 + "\t" + miner.counterTotalRecursions + "\t" + miner.counterNonPivotTransitionsSkipped + "\t" +
				miner.counterMaxPivotUsed + "\t" + result.getCount() + "\t" + result.getTotalFrequency();
		System.out.println(out);
