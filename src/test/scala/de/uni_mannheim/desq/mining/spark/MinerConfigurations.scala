...
object MinerConfigurations {
    ...
    def desqCount(sigma: Long, gamma: Int, lambda: Int, generalize: Boolean, useFlist: Boolean,
        ...
        desqCount(sigma, patternExpression, useFlist, pruneIrrelevantInputs, useTwoPass)
    }
    ...
    def all(sigma: Long, patternExpression: String): util.List[(String, DesqProperties)] = {
        ...
        allMiners.add(desqCount(sigma, patternExpression, useFlist = false, pruneIrrelevantInputs = false, useTwoPass = false))
        allMiners.add(desqCount(sigma, patternExpression, useFlist = false, pruneIrrelevantInputs = true, useTwoPass = false))
        allMiners.add(desqCount(sigma, patternExpression, useFlist = false, pruneIrrelevantInputs = true, useTwoPass = true))
        allMiners.add(desqCount(sigma, patternExpression, useFlist = true, pruneIrrelevantInputs = false, useTwoPass = false))
        allMiners.add(desqCount(sigma, patternExpression, useFlist = true, pruneIrrelevantInputs = true, useTwoPass = false))
        allMiners.add(desqCount(sigma, patternExpression, useFlist = true, pruneIrrelevantInputs = true, useTwoPass = true))
      // desqDfs test
